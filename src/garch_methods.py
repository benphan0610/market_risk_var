"""GARCH and GJR-GARCH walk-forward forecasts."""
...
def garch_forecasts(returns_pct, asymmetric=False):
    ...
    for i in range(GARCH_WARMUP, len(returns_pct)):
        if (i - GARCH_WARMUP) % GARCH_REFIT_INTERVAL == 0:
            ...
            if asymmetric:
                model = arch_model(window, vol="GARCH", p=1, o=1, q=1, dist="normal")
            else:
                model = arch_model(window, vol="GARCH", p=1, q=1, dist="normal")
